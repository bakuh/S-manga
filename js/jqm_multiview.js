...
$.widget( "mobile.collapsible", $.mobile.widget, {
	...
	_create: function() {
		...
		if ( collapsibleSet.length ) {
			// // xxx frequent - check for direction
		...
		collapsible
			.bind( "expand collapse", function( event ) {
				if ( !event.isDefaultPrevented() ) {
					...
					// xxx frequent - prevent last collapsible in set to get ui-corner-bottom
	...
	$.widget( "mobile.popup", $.mobile.widget, {
		...
		_open: function( options ) {
			...
			if ( this.options.overlayTheme && androidBlacklist ) {
				...
				types of input. These issues are reminiscent of previously uncovered bugs in older versions of Androidfs native browser:
...
$( document ).delegate( ":jqmData(role='listview')", "listviewcreate", function() {
	...
	var list = $( this ),
		// XXX frequent relate lists 
		relate = $( this ).jqmData("relate"),
		// XXX end
		listview = list.data( "listview" );
	...
	var wrapper = $( "<form>", {
		...
		.bind( "keyup change", function() {
			...
			if ( isCustomFilterCallback || val.length < lastval.length || val.indexOf( lastval ) !== 0 ) {
				...
				// XXX Frequent - add related list
				listItems = typeof relate == "undefined" ? list.children() 
										: list.children().add( $('ul[data-related="'+relate+'"]').children() );
				// XXX end
			} else {

				// Only chars added, not removed, only use visible subset
				// XXX Frequent - add related list
				listItems = typeof relate == "undefined" ? list.children( ":not(.ui-screen-hidden)" )
										: list.children( ":not(.ui-screen-hidden)" ).add( $('ul[data-related="'+relate+'"]').children( ":not(.ui-screen-hidden)" ) );					
				// XXX end
			}
